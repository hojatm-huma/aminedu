  ...
  return (
    <nav dir="rtl" className="px-3 py-6 space-y-1">
      {NAV_ITEMS.map((item) => {
        ...
        const idleCls =
          "text-[#7A9BB5] hover:bg-[#E8F0FA] hover:text-[#3E66A8]";
          ...
          >
            ...
            {hasSubtabs &&
              (flyout ? (
                ...
                >
                  <div
                    className="bg-white rounded-[14px] shadow-[0_8px_32px_rgba(0,0,0,0.13)] border border-[#EEF0F4] py-2 min-w-[200px]"
                    dir="rtl"
                  >
                    {/* Arrow tip */}
                    ...
                    {item.subtabs.map((sub) => {
                      ...
                      return (
                        ...
                        >
                          {isSubActive && (
                            <span className="w-1.5 h-1.5 rounded-full bg-[#3E66A8] ml-2 shrink-0" />
                          )}
                          {sub.label}
                        </Link>
                      );
                ...
                <div
                  className={`overflow-hidden transition-all duration-300 ease-in-out ${isHovered ? "max-h-80 opacity-100 mt-1" : "max-h-0 opacity-0"}`}
                >
...
function ProfileAvatar({
  ...
  return (
    ...
    >
      <span className="text-[13px] font-bold text-[#4A5568] hidden sm:block">
        {name}
      </span>
    ...
    <main className="min-h-screen bg-[#F4F7FB]">
      <div className="flex min-h-screen">
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          ...
          <header
            dir="rtl"
            className="h-[64px] bg-white border-b border-[#EEF0F4] flex items-center px-4 sm:px-6 gap-3 shrink-0"
          >
            ...
            <div className="relative" onClick={(e) => e.stopPropagation()}>
              ...
              {/* Dropdown */}
              <div
                className={`absolute top-full right-0 z-50 pt-2 transition-all duration-200 ${userMenuOpen ? "opacity-100 pointer-events-auto translate-y-0" : "opacity-0 pointer-events-none -translate-y-1"}`}
              >
                <div
                  dir="rtl"
                  className="rounded-[14px] bg-white shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-[#EEF0F4] p-1 w-[160px]"
                >
                  <button
            ...
            <div className="flex-1 flex justify-center px-2">
              <div className="w-full max-w-[560px] hidden sm:block">
                <div className="h-9 rounded-[10px] bg-[#F4F7FB] flex items-center px-3 gap-2">
                  ...
                  <input
                    className="w-full bg-transparent outline-none text-[13px] text-[#587181] placeholder:text-[#A0AEC0]"
                    placeholder="جستجو..."
                  />
                </div>
              </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="w-9 h-9 rounded-[10px] bg-[#F4F7FB] flex items-center justify-center hover:bg-[#E8F0FA] transition"
                aria-label="اعلان‌ها"
              >
                <svg
                  width="18"
        ...
        <aside className="hidden lg:flex flex-col w-[240px] bg-white border-l border-[#EEF0F4] shrink-0">
          ...
          <div className="h-[64px] flex items-center justify-center gap-3 border-b border-[#EEF0F4] px-4">
            ...
            <span className="font-bold text-[20px] text-[#1A2B45]">
              پلتفرم امین
            </span>
          </div>
        ...
        >
          <div className="flex items-center justify-between px-4 pb-3 border-b border-[#EEF0F4]">
            <button
              onClick={() => setSidebarOpen(false)}
              className="w-8 h-8 rounded-[8px] bg-[#F4F7FB] flex items-center justify-center text-[20px] leading-none"
            >
              ×
            </button>
            <div className="flex items-center gap-2">
              <span className="font-bold text-[18px] text-[#1A2B45]">
                پلتفرم امین
              </span>
        ...
        >
          ...
          >
            <h3 className="text-[20px] font-bold mb-4">
              {openNotifModal ? "اعلان‌ها" : "پیام‌ها"}
            </h3>
